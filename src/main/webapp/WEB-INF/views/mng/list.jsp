...
$(function(){
	...
	const dayTable = document.getElementById('dayTable');
			...
			$.ajax({
				...
				success:function(result){
				    ...
				    $.each(result, function(idx, franchiseVO) {
				        str += "<option class='fran' value='" + franchiseVO.frcsNo + "'>" + franchiseVO.frcsNm + "</option>";
				    });
				    $(".frnSel").html(str);
				}
			});
 	...
 	$('.auto').on('click',function(){
 		...
 		$('.sel').val($('.locat').eq(1).val()).change();
 		$('#mngDt').val('2024-07-17');
 		$('#mngPp').val('[7월] 점포 위생 및 청결 상태 정기 평가 방문');
 		$('#empNo').val($('.selEmp').eq(1).val());
 		setTimeout(function() {
	 		$('.frnSel').val($('.fran').eq(1).val());
	    }, 50); 		
 	});
	...
	<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	    <div class="modal-dialog modal-lg" role="document">
	        <div class="modal-content">
					    ...
					    <div class="card-body">
					    	...
					    	<select class="form-control" id="empNo" name="empNo">
					            ...
					            <c:forEach var="employeeVO" items="${employeeVOList}" varStatus="stat">
					                <option class="selEmp" value="${employeeVO.empNo}">${employeeVO.empNm} (
					                <c:choose>
								...
								<div class="col-md-6">
									...
									<select class="form-control sel">
										...
										<c:forEach var="location" items="${locationVOList}"
											varStatus="stat">
											<option class="locat" value="${location.comcdGroupcd}">${location.comcdCdnm}</option>
										</c:forEach>
									</select>
								</div>
								<div class="col-md-6">
	            ...
	            <div class="modal-footer"><button class="btn btn-black auto" type="button" >자동등록</button><button class="btn btn-primary submit" type="button" data-bs-dismiss="modal">등록</button><button class="btn btn-secondary" type="button" data-bs-dismiss="modal">닫기</button></div>
	        </div>
	    </div>
	</div>
